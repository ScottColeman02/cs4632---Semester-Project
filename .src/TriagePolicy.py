...
class TriagePolicy:
    ...
    def standard_esi(self, patient,simulation):
        comp_cat = patient.comp_cat
        chief_comp = patient.chief_comp
        sev = patient.severity

        if comp_cat in base_2:
            patient.conscious = simulation.rand.choice([True,False])
            if not patient.conscious:
                return 1, simulation.rand.uniform(0,2)
            if chief_comp in base_2_inv and (sev >= 4 and sev <= 6):
                return 3, simulation.rand.uniform(3,5)
            elif chief_comp in base_2_inv and (sev >= 0 and sev <=3):
                return 4, simulation.rand.uniform(2,4)
        elif comp_cat in base_3:
            if sev >= 7:
                return 2, simulation.rand.uniform(1,3)
            elif sev <=3:
                return simulation.rand.integers(4,6), simulation.rand.uniform(4,6)
            
        return simulation.rand.integers(2,5), simulation.rand.uniform(4,6)
